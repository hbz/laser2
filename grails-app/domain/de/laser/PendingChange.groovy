...
class PendingChange {
    ...
    static PendingChange construct(Map<String,Object> configMap) throws CreationException {
        if((configMap.target instanceof Package || configMap.target instanceof TitleInstancePackagePlatform || configMap.target instanceof TIPPCoverage ||
                configMap.target instanceof Subscription || configMap.target instanceof License || configMap.target instanceof CostItem)) {
            PendingChange pc
            String targetClass
            ...
            if(targetClass) {
                if(configMap.prop) {
                    ...
                    List<PendingChange> pendingChangeCheck = executeQuery('select pc from PendingChange pc where pc.status in (:processed) and pc.'+targetClass+'.id :target and pc.targetProperty = :prop',changeParams+[processed:[RDStore.PENDING_CHANGE_ACCEPTED,RDStore.PENDING_CHANGE_PENDING,RDStore.PENDING_CHANGE_HISTORY]])
                    ...
                    List<PendingChange> pendingChangeCheck = executeQuery('select pc from PendingChange pc where pc.status in (:processed) and pc.'+targetClass+'.id = :target and pc.msgToken = :msgToken',changeParams+[processed:[RDStore.PENDING_CHANGE_ACCEPTED,RDStore.PENDING_CHANGE_PENDING,RDStore.PENDING_CHANGE_HISTORY]])
                    if(pendingChangeCheck)
                        return pendingChangeCheck[0]
                    else pc = new PendingChange()
