...
class PendingChangeService extends AbstractLockableService {
    ...
    Map<String,Object> getChanges(LinkedHashMap<String, Object> configMap) {
        ...
        List queryClauses = []
        String query1 = "select pc from PendingChange pc where pc.owner = :contextOrg and pc.status in (:status)",
        query2 = 'select pc.msgToken,pc.pkg,pc.tipp.pkg,pc.tippCoverage.tipp.pkg from PendingChange where pc.pkg in (:packages) or pc.tipp.pkg in (:packages) or pc.tippCoverage.tipp.pkg in (:packages) and pc.msgToken != null group by pc.pkg'
        Map<String,Object> query1Params = [contextOrg:configMap.contextOrg, status:[RDStore.PENDING_CHANGE_ACCEPTED,RDStore.PENDING_CHANGE_PENDING]],
        ...
        List tokensOnly = PendingChange.executeQuery(query2,query2Params)
        /*
